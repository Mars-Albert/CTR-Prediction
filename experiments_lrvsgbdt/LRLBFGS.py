...
train.communicate()
cmd='vw {path}train.vw  -f {path}model --bfgs -c -k --passes 25 -b 20  --loss_function logistic --l2 25 --termination 1e-5  --holdout_off'.format(path=result_path)

subprocess.call(cmd,shell=True,stdout=subprocess.PIPE)

test.communicate()
cmd='vw {path}test.vw  -t -i {path}model  -p {path}preds.txt -P 500 --loss_function logistic'.format(path=result_path)
subprocess.call(cmd,shell=True,stdout=subprocess.PIPE)
