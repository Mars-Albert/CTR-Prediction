___author__ = 'mars'
import sys,subprocess,os
import xgboost as xgb

if len(sys.argv) != 3:
    print('wrong arg')
    exit(1)

data_path=sys.argv[1]
result_path=sys.argv[2]

cmd='python csv2xgboost_onehotencoding.py {0} {1}'.format(data_path,result_path)
csv2vw=subprocess.call(cmd,shell=True,stdout=sys.stdout)


train_data=xgb.DMatrix(result_path+'train.sparse')
test_data=xgb.DMatrix(result_path+'test.sparse')



param = {'bst:max_depth':10, 'bst:min_child_weight':5, 'bst:eta':0.5, 'silent':0, 'objective':'binary:logistic','eval_metric':'logloss' }


param['nthread'] = 4
plst = param.items()
evallist=[(train_data,'train')]

num_round = 30

bst = xgb.train( plst, train_data, num_round, evallist)


bst.dump_model(result_path+'dump.raw.txt')


ypred=bst.predict(test_data)

output=open(result_path+'submission.csv','w')
output.write('Id,Predicted\n')
for p in ypred:
    output.write('{0},{1}\n'.format('anything',p))

output.close()